...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const addAccommodationTotalToBudgetBtn = checkElement('add-accommodation-total-to-budget-btn'); // NUOVO
    ...
    const addItineraryTotalToBudgetBtn = checkElement('add-itinerary-total-to-budget-btn'); // NUOVO
    ...
    const handleCalculateAndAddTotalCostToBudget = async (itemTypeKey, categoryName) => {
        if (!currentTripId) { showToast("Seleziona un viaggio.", "error"); return; }
        const trip = findTripById(currentTripId);
        if (!trip || !Array.isArray(trip[itemTypeKey])) {
            showToast(`Errore dati ${categoryName.toLowerCase()}.`, "error");
            return;
        }

        let totalCost = 0;
        trip[itemTypeKey].forEach(item => {
        ...
        if (totalCost <= 0) {
            showToast(`Nessun costo valido da aggiungere per ${categoryName.toLowerCase()}.`, "info");
            return;
        }

        const success = await addCostToBudget(
            categoryName, 
            `Totale Costi ${categoryName} (del ${formatDate(new Date().toISOString().slice(0,10))})`,
            totalCost
        );
        if(success) {
            showToast(`Costo ${categoryName.toLowerCase()} (${formatCurrency(totalCost)}) aggiunto al budget!`, 'success');
    ...
    const initAppEventListeners = () => {
        ...
        formsToListen.forEach(type => {
            ...
            if (formElement) formElement.onsubmit = (e) => handleItemFormSubmit(e, type);
        ...
        // --- NUOVI EVENT LISTENER PER I PULSANTI BUDGET ---
