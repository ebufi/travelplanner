document.addEventListener('DOMContentLoaded', () => {
    ...
    const formatDisplayLink = (link) => { if (!link) return ''; try { new URL(link); // Valida URL
            return `<a href="${link}" target="_blank" rel="noopener noreferrer" class="external-link">${link.length > 30 ? link.substring(0, 27) + '...' : link} <i class="fas fa-external-link-alt"></i></a>`; } catch (_) { return link; // Se non è URL, mostra come testo
        } };
    ...
    const renderTripList = () => { const searchTerm = currentSearchTerm.trip.toLowerCase(); tripListUl.innerHTML = ''; const sortedTrips = trips .filter(trip => !trip.isTemplate) // Filtra via i template di default
            .sort((a, b) => (a?.name || '').localeCompare(b?.name || '')); sortedTrips.forEach(trip => { if (!trip || !trip.id) return; const tripNameLower = (trip.name || '').toLowerCase(); const destinationLower = (trip.destination || '').toLowerCase(); const isVisible = !searchTerm || tripNameLower.includes(searchTerm) || destinationLower.includes(searchTerm); const li = document.createElement('li'); li.dataset.tripId = trip.id; li.innerHTML = `<span>${trip.name || 'Senza Nome'} (${formatDate(trip.startDate)} - ${formatDate(trip.endDate)})</span> <button class="btn-delete-trip" data-trip-id="${trip.id}" title="Elimina"><i class="fas fa-trash-alt"></i></button>`; if (trip.id === currentTripId) li.classList.add('active'); if (!isVisible) li.classList.add('hidden'); // Nascondi se non matcha ricerca
            li.addEventListener('click', (e) => { if (!e.target.closest('.btn-delete-trip')) selectTrip(trip.id); }); li.querySelector('.btn-delete-trip').addEventListener('click', (e) => { e.stopPropagation(); handleDeleteTrip(trip.id); }); tripListUl.appendChild(li); }); noTripsMessage.style.display = sortedTrips.length === 0 ? 'block' : 'none'; };
    const selectTrip = (id) => { if (currentTripId === id && tripDetailsAreaDiv.style.display !== 'none') return; currentTripId = id; const trip = findTripById(id); if (trip) { // Reset filtri/sort specifici del viaggio precedente
    ...
    const handleItemFormSubmit = (e, listType) => { e.preventDefault(); if (!currentTripId) return; const trip = findTripById(currentTripId); if (!trip) return; const currentEditId = editingItemId[listType]; let itemData = {}; let list = []; let listOwner = trip; let renderFn; // Selezione lista e funzione render
        ...
        try { switch (listType) { case 'participant': if (!participantNameInput.value.trim()) throw new Error("Nome partecipante richiesto."); itemData = { name: participantNameInput.value.trim(), notes: participantNotesInput.value.trim() || null, extraInfo: participantExtraInfoTextarea.value.trim() || null }; break; case 'reminder': // NUOVO
                    if (!reminderDescriptionInput.value.trim()) throw new Error("Descrizione promemoria richiesta."); itemData = { description: reminderDescriptionInput.value.trim(), dueDate: reminderDueDateInput.value || null, status: reminderStatusSelect.value }; break; case 'transport': // ... validazione ...
                    itemData = { type: transportTypeSelect.value, description: transportDescriptionInput.value.trim(), departureLoc: transportDepartureLocInput.value.trim() || null, departureDateTime: transportDepartureDatetimeInput.value || null, arrivalLoc: transportArrivalLocInput.value.trim() || null, arrivalDateTime: transportArrivalDatetimeInput.value || null, bookingRef: transportBookingRefInput.value.trim() || null, cost: transportCostInput.value === '' ? null : parseFloat(transportCostInput.value), notes: transportNotesInput.value.trim() || null, link: transportLinkInput.value.trim() || null }; // ... validazione costo ...
                    break; case 'accommodation': // ... validazione ...
                    itemData = { name: accommodationNameInput.value.trim(), type: accommodationTypeSelect.value, address: accommodationAddressInput.value.trim() || null, checkinDateTime: accommodationCheckinInput.value || null, checkoutDateTime: accommodationCheckoutInput.value || null, bookingRef: accommodationBookingRefInput.value.trim() || null, cost: accommodationCostInput.value === '' ? null : parseFloat(accommodationCostInput.value), notes: accommodationNotesInput.value.trim() || null, link: accommodationLinkInput.value.trim() || null }; // ... validazione costo e date ...
                    break; case 'itinerary': // ... validazione ...
                    itemData = { day: itineraryDayInput.value, time: itineraryTimeInput.value || null, activity: itineraryActivityInput.value.trim(), location: itineraryLocationInput.value.trim() || null, bookingRef: itineraryBookingRefInput.value.trim() || null, cost: itineraryCostInput.value === '' ? null : parseFloat(itineraryCostInput.value), notes: itineraryNotesInput.value.trim() || null, link: itineraryLinkInput.value.trim() || null }; // ... validazione costo e data ...
                    break; case 'budget': // ... validazione ...
                    itemData = { category: budgetCategorySelect.value, description: budgetDescriptionInput.value.trim(), estimated: parseFloat(budgetEstimatedInput.value), actual: budgetActualInput.value === '' ? null : parseFloat(budgetActualInput.value), paidBy: budgetPaidByInput.value.trim() || null, splitBetween: budgetSplitBetweenInput.value.trim() || null }; // ... validazione costo ...
                    break; case 'packing': if (!packingItemNameInput.value.trim()) throw new Error("Nome oggetto richiesto."); const quantity = parseInt(packingItemQuantityInput.value); if (isNaN(quantity) || quantity < 1) throw new Error("Quantità non valida."); itemData = { name: packingItemNameInput.value.trim(), category: packingItemCategoryInput.value.trim() || 'Altro', quantity: quantity }; break; } } catch (error) { showToast(`Errore: ${error.message}`, 'error'); return; } // Aggiunta o Modifica
        if (currentEditId) { const idx = list.findIndex(i => i && i.id === currentEditId); if (idx > -1) { // Mantieni lo stato 'packed' se si modifica un packing item
    ...
    const renderTransportations = (transportItemsInput) => { let items = Array.isArray(transportItemsInput) ? transportItemsInput : []; if (!transportListUl) return; transportListUl.innerHTML = ''; if(noTransportItemsP) noTransportItemsP.style.display = items.length === 0 ? 'block' : 'none'; if (!Array.isArray(items)) return; // Ordinamento
        const sortKey = currentSort.transport; items.sort((a, b) => { if (sortKey === 'type') { return (a?.type || '').localeCompare(b?.type || ''); } if (sortKey === 'cost') { return (b?.cost ?? 0) - (a?.cost ?? 0); // Decrescente
    ...
    const renderItinerary = (itineraryItemsInput) => { let items = Array.isArray(itineraryItemsInput) ? itineraryItemsInput : []; if (!itineraryListUl) return; itineraryListUl.innerHTML = ''; if(noItineraryItemsP) noItineraryItemsP.style.display = items.length === 0 ? 'block' : 'none'; if (!Array.isArray(items)) return; // Filtro
        const searchTerm = currentSearchTerm.itinerary.toLowerCase(); if (searchTerm) { items = items.filter(item => (item.activity?.toLowerCase() || '').includes(searchTerm) || (item.location?.toLowerCase() || '').includes(searchTerm)); } // Ordinamento
    ...
    const renderPackingList = (itemsInput = []) => { let items = Array.isArray(itemsInput) ? itemsInput : []; if (!packingListUl) return; packingListUl.innerHTML = ''; if(noPackingItemsP) noPackingItemsP.style.display = items.length === 0 ? 'block' : 'none'; if (!Array.isArray(items)) return; // Filtro
        ...
        const sortKey = currentSort.packing; items.sort((a, b) => { if (sortKey === 'category') { return (a?.category || '').localeCompare(b?.category || '') || (a?.name || '').localeCompare(b?.name || ''); // Ordina per categoria, poi nome
            } if (sortKey === 'status') { return (a.packed === b.packed)? 0 : a.packed? 1 : -1; // Non imballati prima
            } return (a?.name||'').localeCompare(b?.name||''); // Default nome
        }); // Raggruppamento per Categoria (se ordinato per categoria)
        if (sortKey === 'category') { const grouped = items.reduce((acc, item) => { const cat = item.category || 'Altro'; if (!acc[cat]) acc[cat] = []; acc[cat].push(item); return acc; }, {}); const sortedCategories = Object.keys(grouped).sort(); packingListUl.innerHTML = ''; // Svuota di nuovo
            ...
            items.forEach(item => packingListUl.appendChild(createPackingListItem(item))); } if (!editingItemId.packing) { addPackingItemForm.reset(); packingItemQuantityInput.value = 1; // Resetta quantità a 1
        } };
    ...
    const handleTogglePacked = (itemId, isPacked) => { if (!currentTripId) return; const trip = findTripById(currentTripId); if (!trip) return; trip.packingList = Array.isArray(trip.packingList) ? trip.packingList : []; const idx = trip.packingList.findIndex(i => i && i.id === itemId); if (idx > -1) { trip.packingList[idx].packed = isPacked; saveTrips(); // Non serve re-render completo se si ordina per stato? Sì, serve.
    ...
    const init = () => {
        ...
        tripDetailsAreaDiv.addEventListener('click', (e) => { const editBtn = e.target.closest('.btn-icon.edit'); const deleteBtn = e.target.closest('.btn-icon.delete'); const packingCheckbox = e.target.closest('.packing-checkbox'); // Modifica
            ...
            else if (packingCheckbox) { const itemId = packingCheckbox.dataset.itemId; if(itemId) handleTogglePacked(itemId, packingCheckbox.checked); // Non serve aggiornare classe qui, lo fa renderPackingList
            } });
        ...
        // NUOVO: Listener per Ricerca Interna
