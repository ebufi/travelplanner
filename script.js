...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const handleItemFormSubmit = async (e, listType) => { e.preventDefault(); if (!currentTripId) return; let trip = JSON.parse(JSON.stringify(findTripById(currentTripId))); if (!trip) return; const form = e.target; const currentEditId = editingItemId[listType]; let itemData = {}; try { switch (listType) { case 'participant': itemData = { name: form.querySelector('#participant-name').value.trim(), notes: form.querySelector('#participant-notes').value.trim() }; if(!itemData.name) throw new Error("Il nome del partecipante è obbligatorio."); break; case 'reminder': itemData = { description: form.querySelector('#reminder-description').value.trim(), dueDate: form.querySelector('#reminder-due-date').value, status: form.querySelector('#reminder-status').value }; if(!itemData.description) throw new Error("La descrizione del promemoria è obbligatoria."); break; case 'transport': itemData = { type: form.querySelector('#transport-type').value, description: form.querySelector('#transport-description').value.trim(), departureLoc: form.querySelector('#transport-departure-loc').value.trim(), departureDateTime: form.querySelector('#transport-departure-datetime').value, arrivalLoc: form.querySelector('#transport-arrival-loc').value.trim(), arrivalDateTime: form.querySelector('#transport-arrival-datetime').value, bookingRef: form.querySelector('#transport-booking-ref').value.trim(), cost: safeToNumberOrNull(form.querySelector('#transport-cost').value), notes: form.querySelector('#transport-notes').value.trim() }; if(!itemData.description) throw new Error("La descrizione del trasporto è obbligatoria."); break; case 'accommodation': itemData = { name: form.querySelector('#accommodation-name').value.trim(), type: form.querySelector('#accommodation-type').value, address: form.querySelector('#accommodation-address').value.trim(), checkinDateTime: form.querySelector('#accommodation-checkin').value, checkoutDateTime: form.querySelector('#accommodation-checkout').value, bookingRef: form.querySelector('#accommodation-booking-ref').value.trim(), cost: safeToNumberOrNull(form.querySelector('#accommodation-cost').value), notes: form.querySelector('#accommodation-notes').value.trim() }; if(!itemData.name) throw new Error("Il nome dell'alloggio è obbligatorio."); break; case 'itinerary': itemData = { day: form.querySelector('#itinerary-day').value, time: form.querySelector('#itinerary-time').value, activity: form.querySelector('#itinerary-activity').value.trim(), location: form.querySelector('#itinerary-location').value.trim(), bookingRef: form.querySelector('#itinerary-booking-ref').value.trim(), cost: safeToNumberOrNull(form.querySelector('#itinerary-cost').value), notes: form.querySelector('#itinerary-notes').value.trim() }; if(!itemData.activity || !itemData.day) throw new Error("Giorno e attività sono obbligatori."); break; case 'budget': itemData = { category: form.querySelector('#budget-category').value, description: form.querySelector('#budget-description').value.trim(), estimated: safeToNumberOrNull(form.querySelector('#budget-estimated').value), actual: safeToNumberOrNull(form.querySelector('#budget-actual').value), paidBy: form.querySelector('#budget-paid-by').value.trim(), splitBetween: form.querySelector('#budget-split-between').value.trim() }; if(!itemData.description || itemData.estimated === null) throw new Error("Descrizione e costo stimato sono obbligatori."); break; case 'packing': itemData = { name: form.querySelector('#packing-item-name').value.trim(), category: form.querySelector('#packing-item-category').value.trim() || 'Altro', quantity: safeToPositiveIntegerOrDefault(form.querySelector('#packing-item-quantity').value) }; if(!itemData.name) throw new Error("Il nome dell'oggetto è obbligatorio."); break; default: throw new Error("Tipo lista non valido"); } } catch (error) { showToast(`Errore: ${error.message}`, "error"); return; } let list; switch (listType) { case 'participant': list = trip.participants = trip.participants || []; break; case 'reminder': list = trip.reminders = trip.reminders || []; break; case 'transport': list = trip.transportations = trip.transportations || []; break; case 'accommodation': list = trip.accommodations = trip.accommodations || []; break; case 'itinerary': list = trip.itinerary = trip.itinerary || []; break; case 'budget': list = trip.budget.items = trip.budget.items || []; break; case 'packing': list = trip.packingList = trip.packingList || []; break; } if (currentEditId) { const idx = list.findIndex(i => i.id === currentEditId); if (idx > -1) { const oldItem = list[idx]; list[idx] = { ...oldItem, ...itemData }; } } else { itemData.id = generateId(listType); if(listType === 'packing') itemData.packed = false; list.push(itemData); } await saveTripToFirestore(trip); resetEditState(listType); };
    const handleDeleteItem = async (listType, itemId) => { if (!currentTripId) return; let trip = JSON.parse(JSON.stringify(findTripById(currentTripId))); if (!trip) return; let list; let itemName = "voce"; switch (listType) { case 'participant': list = trip.participants; itemName = "partecipante"; break; case 'reminder': list = trip.reminders; itemName = "promemoria"; break; case 'transport': list = trip.transportations; itemName = "trasporto"; break; case 'accommodation': list = trip.accommodations; itemName = "alloggio"; break; case 'itinerary': list = trip.itinerary; itemName = "attività"; break; case 'budget': list = trip.budget.items; itemName = "spesa"; break; case 'packing': list = trip.packingList; itemName = "oggetto"; break; } if (!list) return; const itemIndex = list.findIndex(i => i.id === itemId); if (itemIndex > -1) { const itemDesc = list[itemIndex].name || list[itemIndex].description || list[itemIndex].activity || `ID ${itemId}`; showConfirmationModal(`Elimina ${itemName}`, `Eliminare "${itemDesc}"?`, async () => { list.splice(itemIndex, 1); await saveTripToFirestore(trip); showToast(`${itemName.charAt(0).toUpperCase() + itemName.slice(1)} eliminato/a.`, 'info'); }); } };
    ...
    const initAppEventListeners = () => {
        ...
        // CORREZIONE: Listener aggiunti esplicitamente per ogni form con il suo ID corretto.
        ...
        // Bottoni "Annulla" per ogni form
        const cancelButtons = ['participant', 'reminder', 'transport', 'accommodation', 'itinerary', 'budget', 'packing'];
        cancelButtons.forEach(type => {
